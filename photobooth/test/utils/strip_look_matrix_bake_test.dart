...
void main() {
  ...
  group('bakeStripLookMatricesOntoDataUrls', () {
    ...
    test('clean is a no-op and invalid URLs fail open', () async {
      ...
      final matrix = stripLookColorMatrixValues('classic_warm');
      expect(bakeOneStripLookMatrixDataUrlForTest(raw, matrix), raw);
      expect(bakeOneStripLookMatrixDataUrlForTest('not-a-data-url', matrix),
          'not-a-data-url');
      expect(bakeOneStripLookMatrixDataUrlForTest(raw, const <double>[1]), raw);
    });
    ...
    test('classic_warm lifts red and softens blue on a cool patch', () {
      ...
      // classic_warm: R'≈53, B'≈160 from (40,60,180) — warmer R/B ratio.
