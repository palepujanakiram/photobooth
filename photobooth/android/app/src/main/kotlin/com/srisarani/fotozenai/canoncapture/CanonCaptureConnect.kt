...
internal object CanonCaptureConnect {
    ...
    fun isConnectOutcome(state: ConnectionState): Boolean = when (state) {
    ...
    fun isStalePtpSession(state: ConnectionState): Boolean = when (state) {
        is ConnectionState.Error,
        is ConnectionState.Wedged,
    ...
    fun errorResult(state: ConnectionState): CaptureSessionContract.Result? = when (state) {
        is ConnectionState.PermissionDenied -> CaptureSessionContract.Result.error(
            CaptureSessionContract.ERROR_PERMISSION_DENIED,
            "USB permission was refused for the camera",
        )

        is ConnectionState.NoDevice -> CaptureSessionContract.Result.error(
            CaptureSessionContract.ERROR_NO_DEVICE,
            "No camera found. Check the cable and that the camera is switched on.",
        )

        is ConnectionState.NoUsbHostSupport -> CaptureSessionContract.Result.error(
