...
class AndroidCameraCameraX extends CameraPlatform {
  ...
  /// Preview use case: lower bound than capture to reduce preview buffer RAM.
  ResolutionSelector? _previewResolutionSelector;

  /// Image capture use case: matches [MediaSettings.resolutionPreset] (full quality).
  ...
  Future<int> createCameraWithSettings(
    ...
    // Preview and ImageAnalysis use one step lower than capture preset so the
    // live texture allocates less RAM; stills use the full preset (see
    // https://developer.android.com/media/camera/camerax/configuration).
    final ResolutionPreset? capturePreset = mediaSettings?.resolutionPreset;
    _previewResolutionSelector = _getResolutionSelectorFromPreset(
      _previewPresetForCapturePreset(capturePreset),
    );
    _captureResolutionSelector = _getResolutionSelectorFromPreset(capturePreset);
  ...
  /// Maps the capture [preset] to a lower preset for [Preview] / [ImageAnalysis]
  /// so preview buffers use less RAM while [ImageCapture] keeps full quality.
  ResolutionPreset? _previewPresetForCapturePreset(ResolutionPreset? preset) {
